---
title: "intervention_analysis: Montreal"
author: "H"
...
intervention2 <- intList[[targetCMA2]]
rm(intList)
...
dfIntersect_annual_2011 <- st_read(conObesity, layer =  c("hiroshi", "dfintersect_annual_2011"), geometry_column ="overlapgeom_20")
dfIntersect_annual_2016 <- st_read(conObesity, layer =  c("hiroshi", "dfintersect_annual_2016"), geometry_column ="overlapgeom_20")
dfIntersect_annual_2021 <- st_read(conObesity, layer =  c("hiroshi", "dfintersect_annual_2021"), geometry_column ="overlapgeom_20")
...
#ggplot(merged_2011) +geom_sf(aes(fill= exist_category))
#ggplot(merged_2016) +geom_sf(aes(fill= exist_category))
#ggplot(merged_2021) +geom_sf(aes(fill= exist_category))
...
merged_year <- rbind( 
  merged_2021 %>% 
    #select(exist_category, starts_with("i_"), starts_with("prevalent_")) %>% 
...
### Map of intervention by year, based on 2021 DAs
...
merged_2021  %>% 
  ...
  group_by(exist_category, exist_2011, exist_2016, exist_2021) %>%
  summarise(DAcount = n(), .groups = "drop") 
...
road <- bind_rows(
  dbGetQuery(conObesity, "Select * from hiroshi.routing_annual_mtl_2011") %>% mutate(yearRouting ="2011"),
  dbGetQuery(conObesity, "Select * from hiroshi.routing_annual_mtl_2016") %>% mutate(yearRouting ="2016"),
  dbGetQuery(conObesity, "Select * from hiroshi.routing_annual_mtl_2021") %>% mutate(yearRouting ="2021")
)
...
road %<>% mutate(gid_area = as.numeric(gid_area))
dfNetwork <- 
  dfNetwork %>%

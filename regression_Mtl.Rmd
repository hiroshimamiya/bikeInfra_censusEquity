---
title: "Regression analysis: Montreal"
author: "Hiroshi"
...
fileLoc <- "data/Mtl"
targetCMA <- "Mtl"
targetCMA2 <- "Mtl_other"
resultLoc <- "result/Mtl"
...
# remove some varaibles that have too many zeros and thus block categolization by percentile 
# Aboriginal var 
...
Large number of zero proportions in Montreal DA 
...
modelDat <-  dfNetwork %>% 
  ...
  mutate(min = scale(log(min), scale = T), popDensity_per1000 = scale(log(popDensity_per1000), scale = T))

```


#### Spatial weight matrix, isolated (incorrect) and connected graphs (valid for most spatial mixed effect models)    
For valid results, weight matrices should have:     
- **symmetric entry**   
- **no isolated area (no zero in rowsum)**   
- **no isolated graph: rivers cannot divide the spatial window into several groups (shores)**     
...
### Dianognosis -------------------------------------------
#head(wList[[1]]$Wnb) # list of areas showing neighbor ID
#summary(wList[[1]]$Wnb) # Note that one region has no link (neighbour) -- is this realistic? 
#is.symmetric.nb(wList[[1]]$Wnb) # Neighborhood needs to be symmetric (i.e. area i is neighbour of j, and vice versa)
...
plot(wList[[1]]$obj,  las=1, main='Connectivity by Shared Borders, with fake bridges connecting graphs', sub = "Correct definition of neighbourhood for most spatial analysis",border = "gray")
plot(wList[[1]]$Wnb_mod, coordinates(wList[[1]]$obj), pch = 0.1, cex = 0.1, points=FALSE, add=TRUE, col="black") #old (and invalid) spatial relationship
```
...
if(bool_runSpatial){
  ...
  for(j in c("2011", "2016", "2021")){
    for(i in modelCol){
      ...
      fitListLmMonoCAR[[j]][[i]] <- S.CARleroux(formula=log(y)~., data=d, family="gaussian", W=wList[[j]]$W_mod, burnin=5000, n.sample=100000, thin=10, n.chains=3, n.cores=3)
      fitListLmMonoCAR[[j]][[i]]$family = "logGaussianCAR"
      
      fitListLmMonoNoCAR[[j]][[i]] = S.glm(formula=log(y)~., data=d, family="gaussian", burnin=5000, n.sample=30000, thin=10, n.chains=3, n.cores=3)
...
modelDatCat <-rbind(
  modelDatCat %>% 
    ...
    mutate(across(all_of("aboriginal") , ~ funcQuantile(.x, q=25))),
  modelDatCat %>% 
    filter(yearPrevalent == "2016") %>% 
    mutate(across(all_of("aboriginal") , ~ funcQuantile(.x, q=4))),
  modelDatCat %>% 
...
if(bool_runSpatial_categorySES){
  ...
  for(j in c("2011", "2016", "2021")){
    for(i in modelCol){
      ...
      fitListLmMonoCAR_cat[[j]][[i]] <- S.CARleroux(formula=log(y)~., data=d, family="gaussian", W=wList[[j]]$W_mod, burnin=5000, n.sample=100000, thin=25, n.chains=3, n.cores=3)
      fitListLmMonoCAR_cat[[j]][[i]]$family = "logGaussianCAR_cat"
      
      fitListLmMonoNoCAR_cat[[j]][[i]] = S.glm(formula=log(y)~., data=d, family="gaussian", burnin=5000, n.sample=30000, thin=25, n.chains=3, n.cores=3)
...
if(bool_runSpatialBin){
  ...
  for(j in c("2011", "2016", "2021")){
    for(i in modelCol){
      ...
      fitListBinMonoNoCAR[[j]][[i]] = S.glm(formula=y~., data=d, family="binomial", burnin=15000, n.sample=150000, thin=10, n.chains=3, n.cores=3, trials = rep(1,nrow(d)))
...
varDescription <- readRDS("~/projects/intervention_sdoh/data/varDescription.rds")
a <- gsub("i_", "", varDescription$varName)
b <- varDescription$shortDesc
c <- tb$Var
...
tb %>% 
  funcMakeTable(Title = "Categorical (tertile) SES for each model, non spatial model", rowlines = c(3, 6, 9, 12, 15, 18, 21, 24))
...
ggsave(paste(resultLoc, "/OLS.jpg", sep = ""), height = 10, width=9.5, units = "in")
...
ggsave(paste(resultLoc, "/OLS_spatial.jpg", sep = ""), height = 10, width=9.5, units = "in")
...
fitListLmMonoCAR_cat[["2011"]][["visMin"]]$samples$tau2[[2]] %>%  plot(main = expression(tau^2))
fitListLmMonoCAR_cat[["2011"]][["young"]]$samples$tau2[[2]] %>%  plot(main = expression(tau^2))
fitListLmMonoCAR_cat[["2011"]][["tenant"]]$samples$tau2[[2]] %>%  plot(main = expression(tau^2))
fitListLmMonoCAR_cat[["2011"]][["LICO"]]$samples$tau2[[2]] %>%  plot(main = expression(tau^2))

fitListLmMonoCAR_cat[["2011"]][["visMin"]]$samples$nu[[2]] %>%  plot(main = expression(nu))
fitListLmMonoCAR_cat[["2011"]][["young"]]$samples$nu[[2]] %>%  plot(main = expression(nu))
fitListLmMonoCAR_cat[["2011"]][["tenant"]]$samples$nu[[2]] %>%  plot(main = expression(nu))
fitListLmMonoCAR_cat[["2011"]][["LICO"]]$samples$nu[[2]] %>%  plot(main = expression(nu))

fitListLmMonoCAR_cat[["2011"]][["visMin"]]$samples$rho[[2]] %>%  plot(main = expression(rho))
fitListLmMonoCAR_cat[["2011"]][["young"]]$samples$rho[[2]] %>%  plot(main = expression(rho))
fitListLmMonoCAR_cat[["2011"]][["tenant"]]$samples$rho[[2]] %>%  plot(main = expression(rho))
fitListLmMonoCAR_cat[["2011"]][["LICO"]]$samples$rho[[2]] %>%  plot(main = expression(rho))

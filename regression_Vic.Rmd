---
title: "Regression analysis: Victoria"
author: "Hiroshi"
...
```{r}
f <- formula(y ~ visMin + aboriginal +educ_15 + move_5yrs + LICO + rec_imm + young + old + tenant + popDensity_per1000)
...
for(i in gsub("i_", "", dfVarDesc$varName)){
  ...
  pList[[i]] <- ggplot() +
    geom_histogram(data = d, aes(x=x), bins=20) + 
    ggtitle(i) 
}
...
#modelDat <-  dfNetwork %>% 
##  dplyr::select(all_of(modelCol), popDensity_per1000, yearPrevalent, network_distance, prevalent) %>% 
...
modelDat <-  dfNetwork %>% 
  ...
  mutate(popDensity_per1000 = scale(log(popDensity_per1000), scale = T))
...
plot(wList[[1]]$obj,  las=1, main='Connectivity by Shared Borders, with fake bridges connecting graphs and excluded DAs due to missing census variables', sub = "Correct definition of neighbourhood for most spatial analysis",border = "gray")
  ...
  for(j in c("2011", "2016", "2021")){
    for(i in modelCol){
      d <- modelDat %>% 
        ...
        dplyr::select(all_of(i), y, popDensity_per1000) %>% 
        st_drop_geometry()
...
modelDatCat <-rbind(
  modelDatCat %>% 
    ...
    mutate(across(all_of("rec_imm") , ~ funcQuantile(.x, q=4))),
  modelDatCat %>% 
    filter(yearPrevalent == "2016") %>% 
    mutate(across(all_of("rec_imm") , ~ funcQuantile(.x, q=3))),
  modelDatCat %>% 
...
if(bool_runSpatial_categorySES){
  ...
  for(j in c("2011", "2016", "2021")){
    for(i in modelCol){
      ...
      fitListLmMonoCAR_cat[[j]][[i]] <- S.CARleroux(formula=log(y)~., data=d, family="gaussian", W=wList[[j]]$W_mod, burnin=5000, n.sample=100000, thin=20, n.chains=3, n.cores=3)
      fitListLmMonoCAR_cat[[j]][[i]]$family = "logGaussianCAR_cat"
      
      fitListLmMonoNoCAR_cat[[j]][[i]] = S.glm(formula=log(y)~., data=d, family="gaussian", burnin=5000, n.sample=30000, thin=20, n.chains=3, n.cores=3)
...
funcGetCategoricalBetCarModel <- function(dList, year){
  lapply(dList[[as.character(year)]], funcGetCoef) %>%
    lapply(data.frame) %>% 
    lapply(function(x){ x %>% dplyr::filter(!rownames(x) %in% c("popDensity_per1000", "nu2", "tau2", "rho", "(Intercept)" ))}) %>%
    bind_rows() %>% 

---
title: "intervention_analysis: VIC"
author: "H"
...
intervention <- st_read(conObesity, layer =  c("hiroshi", "interact_intervention_annual_vic"), geometry_column ="geom")
...
dfIntersect_annual_2011 <- st_read(conObesity, layer =  c("hiroshi", "dfintersect_vic_annual_2011_other"), geometry_column ="overlapgeom_20")
dfIntersect_annual_2016 <- st_read(conObesity, layer =  c("hiroshi", "dfintersect_vic_annual_2016_other"), geometry_column ="overlapgeom_20")
dfIntersect_annual_2021 <- st_read(conObesity, layer =  c("hiroshi", "dfintersect_vic_annual_2021_other"), geometry_column ="overlapgeom_20")

 

 
#library(magrittr)
#dfIntersect_annual_2021 %<>% mutate(GeoUID = as.numeric(GeoUID))
#dfIntersect_annual_2016 %<>% mutate(GeoUID = as.numeric(GeoUID))
#dfIntersect_annual_2011 %<>% mutate(GeoUID = as.numeric(GeoUID))
...
library(ggspatial)
p<- ggplot() + 
  geom_sf(data = merged_2021, size =1, color= "grey70", fill = "white") + 
  geom_sf(data = intervention_agg_2021, aes(color = "Existed in 2011"), lwd= 0.6, alpha = 0.5, show.legend = "line") +
  geom_sf(data = road_new2016, aes(color = "Newly added in 2016"), lwd= 1,alpha = 1.5, show.legend = "line") +
  geom_sf(data = road_new2021, aes(color = "Newly added in 2021"), lwd= 1,alpha = 1.5, show.legend = "line") +
...
Newwork distance, prepeare quintile measurees 
```{r}
...
road <- bind_rows(
  dbGetQuery(conObesity, "Select * from hiroshi.routing_annual_vic_2011") %>% mutate(yearRouting ="2011"),
  dbGetQuery(conObesity, "Select * from hiroshi.routing_annual_vic_2016") %>% mutate(yearRouting ="2016"),
  dbGetQuery(conObesity, "Select * from hiroshi.routing_annual_vic_2021") %>% mutate(yearRouting ="2021")
)
road <- road %>%  
...
dfNetwork <- dfNetwork %>% 
  left_join(merged_year %>% dplyr::select(popDensity_per1000, exist_category, yearPrevalent, GeoUID) %>% st_drop_geometry(), 
...
Make it for long data for plotting 
```{r}
...
Comparison of network and quclidian distance across years, and across the binary status of DA: whether touching bike infra or not 
```{r}
dfNetwork %>%  
  st_drop_geometry() %>% 
...
modelCol <-c("network_distance", "visMin",  "educ_15", "move_5yrs", "LICO", "rec_imm", "young", "old", "tenant", "aboriginal",  "popDensity_per1000")
